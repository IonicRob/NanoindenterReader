...
ImportNewYS = questdlg('Do you need to import new data?','Nanoindentation Data Reader','Yes','No','Yes');

% The following applies if the user selected Yes or the settings have not
% been set before.
if strcmp(ImportNewYS,'Yes') || logical(exist('SettingsDone','var')) == false
...
% The following applies when No is selected but the settings have been
% previously worked with.
if strcmp(ImportNewYS,'No') && logical(exist('SettingsDone','var')) == true
    disp('Using the same data as before!')
end

% The following happens when no option is chosen.
if strcmp(ImportNewYS,'')
...
LoadingMode = false;
cd(LOC_init);
[DataIDName,SaveTime,SavingData,LOC_save] = NanoDataSave(ImageFormatType,LoadingMode,LOC_init,LOC_load,dlg_title,fileNameList);    
